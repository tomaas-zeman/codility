...
#
# def solution(A, B)
#     that, given two non-empty zero-indexed arrays A and B consisting of N integers, returns the number of fish that
#     will stay alive.
#
...
class Fish:
    def __init__(self, size, upstream):
        self.size = size
        self.upstream = upstream


def solution(A, B):
    fishes = list(map(lambda i: Fish(A[i], B[i]), range(len(A))))
    fishes.reverse()
    alive = []
    while len(fishes) > 0:
        current_fish = fishes.pop()
        if len(alive) == 0:
            alive.append(current_fish)
        else:
            stacked_fish = alive.pop()
            if (not stacked_fish.upstream) or (stacked_fish.upstream and current_fish.upstream):
                alive.append(stacked_fish)
                alive.append(current_fish)
            else:
                while True:
                    if stacked_fish.size > current_fish.size:
                        alive.append(stacked_fish)
                        break
                    elif len(alive) == 0 or (stacked_fish.size < current_fish.size and not alive[-1].upstream):
                        alive.append(current_fish)
                        break
                    stacked_fish = alive.pop()
...
test([[[4, 3, 2, 1, 5], [0, 1, 0, 0, 0]]], [2], solution)
