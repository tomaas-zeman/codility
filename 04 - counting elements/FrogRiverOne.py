...
def solution(X, A):
    river_road = [False] * X
    river_road_missing_leaves = X
    for i in range(len(A)):
        falling_leaf_position = A[i] - 1
        if not river_road[falling_leaf_position]:
            river_road[falling_leaf_position] = True
            river_road_missing_leaves -= 1
        if river_road_missing_leaves == 0:
            return i
    return -1


test([[5, [1, 3, 1, 4, 2, 3, 5, 4]], [1, [1]]], [6, 0], solution)
