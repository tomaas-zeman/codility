...
# to find the minimal impact factor of nucleotides contained in the DNA sequence between positions P[K]
# and Q[K] (inclusive).
#
...
def letter_to_impact(letter): return impact.get(letter)


def solution(S, P, Q):
    solution_a = []
    for i in range(0, len(P)):
        solution_a.append(min(map(letter_to_impact, S[P[i]:Q[i] + 1])))
    return solution_a


test([["CAGCCTA", [2, 5, 0], [4, 5, 6]]], [[2, 4, 1]], solution)